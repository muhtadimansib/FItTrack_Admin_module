...
//   { name: "Push-Up", category: "Bodyweight", date: "06/23/2024", addedBy: "Ronald Jim" },
//   { name: "Bicep-Curl", category: "Strength", date: "06/23/2024", addedBy: "Ronald Jim" },
//   { name: "Plank", category: "Bodyweight", date: "06/23/2024", addedBy: "Ronald Jim" },
//   { name: "Triceps Dips", category: "Bodyweight", date: "06/23/2024", addedBy: "Ronald Jim" },
// ];
...
//             <div
//               key={index}
//               className={`rounded-2xl p-4 shadow-md transition-transform hover:scale-105 duration-300 flex items-center gap-4 ${
//     darkMode ? "bg-zinc-800 text-white" : "bg-zinc-100 text-zinc-800"
//   }`}
//             >
//               {stat.icon}
...
//         {/* Recent Exercises */}
//         <div className={`rounded-2xl p-4 shadow-md mt-6 animate-fadeIn ${
//     darkMode ? "bg-zinc-800 text-white" : "bg-zinc-100 text-zinc-800"
//   }`}>
...
//                   <tr key={idx} className={`border-t text-sm ${darkMode ? "border-zinc-700" : "border-zinc-200"}`}>
//                   <td>{ex.name}</td>
//                   <td>{ex.category}</td>
//                   <td>{ex.date}</td>
//                   <td>{ex.addedBy}</td>
...
//           <div className={`rounded-2xl p-4 text-center shadow-md relative hover:scale-105 transition-transform duration-300 flex flex-col items-center gap-2 ${
//     darkMode ? "bg-zinc-800 text-white" : "bg-zinc-100 text-zinc-800"
//   }`}>
...
//           </div>
//           <div className={`rounded-2xl p-4 text-center shadow-md hover:scale-105 transition-transform duration-300 flex flex-col items-center gap-1 ${
//     darkMode ? "bg-zinc-800 text-white" : "bg-zinc-100 text-zinc-800"
//   }`}>
...
//       { icon: <Clock size={22} />, label: "Login activity" },
//       { icon: <BarChart3 size={22} />, label: "Performance" },
//       { icon: <Settings size={22} />, label: "Settings" },
//       { icon: <LogOut size={22} />, label: "Logout", danger: true, onClick: handleLogout },
//     ].map((item, idx) => (
//       <li key={idx}>
//       <a
//         onClick={item.onClick}
//         className={`flex items-center gap-4 px-4 py-3 rounded-lg transition duration-300 cursor-pointer ${
//           item.active
//             ? "bg-orange-500 text-white"
//             : darkMode
//             ? "hover:bg-zinc-800 hover:text-orange-400"
...
//   useEffect(() => {
//     fetch("http://localhost:3000/admin/latest-exercises")
//       .then((res) => res.json())
//       .then((data) => {
...
//         {/* Recent Exercises */}
//          <div
//       className={`rounded-2xl p-4 shadow-md mt-6 animate-fadeIn ${
//         darkMode ? "bg-zinc-800 text-white" : "bg-zinc-100 text-zinc-800"
//       }`}
//     >
//       <h2 className="text-lg font-semibold mb-2">Recent Exercises</h2>
...
import users from "@/lottie icons/users.json"
import workoutPlan from "@/lottie icons/workoutPlans.json"
import exercises from "@/lottie icons/pushUp.json"
import nutritions from "@/lottie icons/Nutrition.json"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const response = await fetch('http://localhost:3000/admin/latestOpenReport');
        const data: ReportType = await response.json(); // 👈 type it!
        setReport(data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchLatestPendingUser = async () => {
      try {
        const response = await fetch("http://localhost:3000/admin/latestPendingUser");
  ...
  const stats = [
    {
      label: "Users",
      value: dashboardData?.totalUsers || 1000,
      icon: <Lottie animationData={users} className="w-10 h-10" loop={true} />,
    },
    {
      label: "Workout Plans",
      value: dashboardData?.totalWorkoutPlans || 0,
      icon: <Lottie animationData={workoutPlan} className="w-16 h-16" loop={true} />,
    },
    {
      label: "Exercises",
      value: dashboardData?.totalExercises || 0,
      icon: <Lottie animationData={exercises} className="w-16 h-16" loop={true} />,
    },
    {
      label: "Nutrition Entries",
      value: dashboardData?.totalNutritionEntries || 0,
      icon: <Lottie animationData={nutritions} className="w-16 h-16" loop={true} />,
    },
  ];
