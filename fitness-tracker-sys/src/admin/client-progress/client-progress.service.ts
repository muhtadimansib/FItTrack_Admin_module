...
async exportProgressAsPDFwithChart(clientId: number, exportPath: string): Promise<Buffer> {
 const { goals, activities, workoutAndDietLogs } = await this.getProgress(clientId);
 ...
 // Fetch client details
  ...
  // 🔹 Use path from query (fallback to 'exports' dir if none provided)
  ...
  doc.fontSize(14).text(`Name: ${client?.name ?? 'N/A'}`);
  doc.text(`Email: ${client?.email ?? 'N/A'}`);
  doc.text(`Age: ${client?.age ?? 'N/A'}`);
  doc.text(`Gender: ${client?.gender ?? 'N/A'}`);
  doc.text(`Height: ${client?.height ?? 'N/A'}`);
  doc.text(`Weight: ${client?.weight ?? 'N/A'}`);
  doc.text(`Medical History: ${client?.medicalHistory ?? 'N/A'}`);
  doc.text(`BMI: ${client?.bmi ?? 'N/A'}`);
...
// }
 ...
 async getAllClientsStats() {
    // Fetch all clients with necessary fields
    const clients = await this.clientRepo.find();

    const result = await Promise.all(
      clients.map(async (client) => {
        // Count completed goals for this client
        const completedGoalsCount = await this.goalRepo.count({
          where: {
            clientId: client.id,
            isCompleted: true,
          },
        ...
        // Average sleep hours from daily activity for this client
        const activities = await this.activityRepo.find({
          where: { clientId: client.id },
        });

        let avgSleepHours: number | string = 'N/A';
        if (activities.length > 0) {
          const totalSleep = activities.reduce((acc, curr) => acc + curr.sleepHours, 0);
          avgSleepHours = parseFloat((totalSleep / activities.length).toFixed(2));
        }

        // Prepare response with fallback 'N/A' for null or undefined
        return {
          id: client.id,
          name: client.name ?? 'N/A',
          email: client.email ?? 'N/A',
          age: client.age !== null && client.age !== undefined ? client.age : 'N/A',
          gender: client.gender ?? 'N/A',
          goalsCompleted: completedGoalsCount,
          bmi: client.bmi !== null && client.bmi !== undefined ? client.bmi : 'N/A',
          averageSleepHours: avgSleepHours,
          phone: client.phone ?? 'N/A',
          address: client.address ?? 'N/A',
          profileImageUrl: client.profileImageUrl ?? 'N/A',
...
async getStepCountChartData(clientId: number) {
  const activities = await this.activityRepo.find({
    where: { client: { id: clientId } },
    order: { date: 'ASC' },
  });
...
async getSleepHoursChartData(clientId: number) {
  const activities = await this.activityRepo.find({
    where: { client: { id: clientId } },
    order: { date: 'ASC' },
  });

  const labels = activities.map(a => new Date(a.date).toISOString().split('T')[0]);
  const sleepHours = activities.map(a => a.sleepHours);

  return { labels, sleepHours };
}

async getSleepEfficiencyVsCaloriesChartData(clientId: number) {
  const activities = await this.activityRepo.find({
    where: { client: { id: clientId } },
    order: { date: 'ASC' },
  });
  ...
  const scatterData = activities.map((a) => {
    const sleepEfficiency = parseFloat(((a.sleepHours / 24) * 100).toFixed(2));
    return { x: a.caloriesBurned, y: sleepEfficiency };
  });
...
async getBubbleChartData(clientId: number) {
  const activities = await this.activityRepo.find({
    where: { client: { id: clientId } },
    order: { date: 'ASC' },
  });
