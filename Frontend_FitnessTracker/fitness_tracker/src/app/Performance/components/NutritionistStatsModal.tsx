...
interface ChartData {
  rating: Array<{ week: string; averageRating: number }>;
  mealPlanCompletion: Array<{ name: string; rate: number }>;
}
...
export default function NutritionistStatsModal({ nutritionistId, onClose }: Props) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [rating, mealPlanCompletion] = await Promise.all([
          fetch(
            `${apiBase}/admin/performance/nutritionist/${nutritionistId}/weekly-rating-chart-jsondata`
          ).then((res) => res.json()),
          fetch(
            `${apiBase}/admin/performance/nutritionist/${nutritionistId}/mealplan-completion-jsondata`
          ).then((res) => res.json()),
        ]);
  ...
  const renderChart = () => {
    ...
    if (!chartData.rating.length || !chartData.mealPlanCompletion.length) {
